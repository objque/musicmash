...
import (
	...
	"github.com/musicmash/musicmash/internal/clients/deezer/artists"
...
func (l *Linker) SearchArtists(userArtists []string) {
	...
	for _, artist := range userArtists {
		dbArtists, err := db.DbMgr.GetArtistFromStore(artist, "deezer")
		if err != nil {
		...
		err = db.DbMgr.EnsureArtistExistsInStore(artist, "deezer", strconv.Itoa(storeArtist.ID))
		if err != nil {
