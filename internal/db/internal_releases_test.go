...
func (t *testDBSuite) TestInternalReleases_GetArtist() {
	...
	expected := &InternalRelease{ID: 1, ArtistID: artistID, Title: vars.ReleaseAlgorithmFloatingIP, Poster: vars.PosterSimple, Released: r, ItunesID: "1000", DeezerID: "2000", SpotifyID: "3000"}
	assert.Equal(t.T(), expected, releases[0])

	r = r.AddDate(0, 0, -1)
	expected = &InternalRelease{ID: 4, ArtistID: artistID, Title: vars.ReleaseArchitectsHollyHell, Poster: vars.PosterSmall, Released: r, ItunesID: "4000", DeezerID: "5000", SpotifyID: "6000"}
